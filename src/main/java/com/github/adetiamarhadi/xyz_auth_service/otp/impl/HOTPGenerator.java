...
public class HOTPGenerator implements OTPGenerator {
    ...
    public String generate(String userUuid, OTPType otpType) {
        ...
        try {
            ...
            return generateOTP(secretKey.getBytes(), Instant.now().toEpochMilli(), OTP_LENGTH, false, -1);

        } catch (Exception e) {

            throw new IllegalStateException("OTP generation failed due to internal error", e);
        }
    }
     ...
     *
     */
    private byte[] hmacSha1(byte[] keyBytes, byte[] text) throws NoSuchAlgorithmException, InvalidKeyException {
